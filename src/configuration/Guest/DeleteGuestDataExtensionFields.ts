...
export class DeleteGuestDataExtensionFields
{
    public async AddTool(server: McpServer) {
        server.tool(
            //name
            "DeleteGuestDataExtensionFields",
            //description
            "Deletes fields from a specific guest data extension in Sitecore CDP using the Guest API.",
            //paramsSchema
            {
                ...
                fields: z.array(z.string()).describe("A list of properties names that should be removed. Example: 'vipMember': true or 'loyaltyTier': 'level2'"),
            },
            //annotations
            {
                title: "Deletes properties from a specific guest data extension in Sitecore CDP", // Human-readable title for the tool
            ...
            //callback
            async ({ guestRef, dataExtensionName, fields }) => 
            {   
                const response = await new CdpClient().MakeRequest(
                    `guests/${guestRef}/extensions/${dataExtensionName}/fields?name=${fields.join(',')}`,
                    'DELETE',
                    null
                );
