...
export class UpdateGuestDataExtension 
{
    public async AddTool(server: McpServer) {
        server.tool(
            //name
            "PartialUpdateGuestDataExtension",
            //description
            "Partially updates a specific guest data extension in Sitecore CDP using the Guest API.",
            //paramsSchema
            {
                ...
                properties: z.array(z.object({key: z.string(), value: z.any()})).describe("Any additional properties may be string or boolean or integer or number to create a key-value pair. Example: 'vipMember': true or 'loyaltyTier': 'level2'"),
            },
            //annotations
            {
                title: "Partially updates a specific guest data extension in Sitecore CDP", // Human-readable title for the tool
            ...
            //callback
            async ({ guestRef, dataExtensionName, properties }) => 
            {     
                const propObject: { [key: string]: any } = {}
                for (const property of properties) {
                    const key = property.key as string;
                    propObject[key] = property.value;
                }
